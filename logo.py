...
def draw_right_lines():
    ...
    for alpha in np.arange(PI / 2, -3 * PI / 2, -angle_step):
        ...
        if 0 <= alpha < PI / 2:
            ...
            first_lines.add(
                Line(end_point, np.array([x, y, 0]), color=LINES_COLOR, stroke_width=0.5, stroke_opacity=1))
...
class Logo(Scene):
    ...
    def draw_center(self):
        ...
        intersection_area = Intersection(circle1, circle2, fill_opacity=1).set_color(SECTOR_COLOR).scale(
            0.9)
